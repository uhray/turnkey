...
proto.launch = function(opts) {
  ...
  cfg.router.use(function(req, res, next) {
    ...
    if (cfg.mockUser && mockToken && !currUser) req.session.mockUserToken = null;
  ...
  cfg.router.get('/turnkey/mockUser/:user', function(req, res) {
    var u = req.user,
        query = {};
  ...
  if (cfg.socialAuth) {
    cfg.router.post('/turnkey/socialAuth', function(req, res) {
        var d = req.body,
            secret;

        if (!d.auth) return res.send('invalid data');
        if (!d.auth.clientId) return res.send('invalid data');
        if (!d.auth.id) return res.send('invalid data');
        if (!d.auth.network) return res.send('invalid data');
        if (!d.auth.token) return res.send('invalid data');

        secret = cfg.socialSecrets[d.auth.network];
        if (!secret) return res.send('network not configured');
        ...
        tools.verifyNetwork(
          ...
          secret,
          function(verified) {
            if (!verified) return res.send('unauthorized');
            ...
            cfg.model.findOne({
              ...
              if (e || !user) {
                ...
                if (!d.create || !cfg.socialCreate) return res.send('unauthorized');
                ...
                cfg.socialCreate(d.create, function(e, d) {
                  ...
                  else if (!d) res.send('unauthorized');
                  else isAuthed(d);
                });
              } else {
