...
proto.launch = function(opts) {
  ...
  cfg.router.use(function(req, res, next) {
    ...
    if (cfg.mockUser && mockToken && token) token = mockToken;
    if (cfg.mockUser && mockToken && !token) req.session.mockUserToken = null;
    ...
    cfg.deserialize(token, function(err, user) {
      ...
      if (!user && cfg.mockUser && mockToken) req.session.mockUserToken = null;
      req.user = user;
