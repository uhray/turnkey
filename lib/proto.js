...
proto.launch = function(opts) {
  ...
  if (!cfg.forgot_mailer) return debug('no forgot mailer set');

  cfg.router.post('/turnkey/forgot', function(req, res) {
    var body = req.body;
    ...
    cfg.find_user(body, function(e, user) {
      ...
      cfg.model.findByIdAndUpdate(id, update, function(e, user) {
        ...
        cfg.forgot_mailer(user, code, function(e) {
          if (e) {
            res.json(self.errorObject(7, 'invalid user'));
  ...
  cfg.router.put('/turnkey/reset', self.createPassword(), function(req, res) {
    var body = req && req.body,
        code = body && body.code,
        turnkey = body && body['turnkey.password'],
        time = new Date().getTime() - cfg.forgot_limit,
        query = {
          'turnkey.forgot.code': code,
...
proto.createPassword = function() {
  var minLength = this.config.min_length,
      self = this;
